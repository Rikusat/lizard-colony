...
const CFG = {
  ...
  //   shape=リングの姿形(省略/"ring"=円弧=原型 / "poly"=多角形枠)。sides=多角形の辺数(3/4/6/8…)。
  //     多角形は角度窓判定と完全一致(頂点=物理半径radii[i]・辺は内側へ僅かに窪む=切れ目角は物理値と厳密一致)。
  //     各リングは物理の独立回転(slitSpinDeg)で枠ごと回る=「回転しあう幾何」。骨格(半径/切れ目角/回転)は不変=意匠のみ。
  ...
  slitSkinByStage: {
    // 基準カット(3惑星・多角形3種で「姿形が変わる」基準を作る)。他7惑星は既定(円環)にフォールバック(展開はRic承認後)。
