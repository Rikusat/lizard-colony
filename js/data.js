...
const CFG = {
  ...
  // ---- UPD-1(2026-09-05 Ric指示「単調さの打破」): ロックオン迎撃 — 照準ウィンドウ ----
  //   鷹の威嚇タップ(hawkTapToScare)・クモの巣ほつし と同系の「raid中ギミック」を地上敵全体へ一般化する。
  //   周期的に敵へHOLOブラケットが灯り、その間にタップ=ひるみ(次の噛みつきを遅らせる=負傷予防)+DPS換算の追撃。
  //   ★無操作なら従来の自動戦闘と完全に同一(加点設計=音の「無音が基準」と同型)。タイミングは到着からの
  //   経過秒の純関数=決定論(Math.random 不使用)。全数値★Ric実機調整。恒久テスト=aim_regression。
  aimOn: true,            // ★falseで機構ごと無効=従来挙動へ完全復帰(可逆)
  aimFirstSec: 4,         // ★到着から最初のウィンドウまでの秒
  aimCycleSec: 7,         // ★ウィンドウの周期(秒)
  aimWindowSec: 1.6,      // ★ウィンドウが開いている長さ(秒)
  aimStunSec: 3.5,        // ★ヒット時のひるみ秒(既存stunT機構=次の噛みつきをこの分遅らせる)
  aimDmgSec: 2.5,         // ★ヒット時の追撃=コロニーDPS×この秒数ぶん(新しい強さの式を作らない)
  aimHitR: 80,            // ★タップ判定半径(px)
  ...
  slitHalfDeg: [54, 28, 16, 10],       // 各円の切れ目の半角(度)。外=54(通過30%=70%弾き)。内へ急に狭め、成功を"食"級に稀少化(§9.5)。
                                       ...
                                       //   ※≈1/3900はS-SLIT-R前の旧実測。初期位相ランダム化後の正値=設計値1/4339(§5t-2・2026-08-27確定)
  ...
  // ---- V6-P5 S2: 飼育槽の環境音(常時鳴る音)。★全数値はRicが ?tune=1#sound で実機判定 ----
  //   SEとは設計思想が異なる: SE=一瞬の主張(物差しはpeak) / 環境音=背景に沈む(物差しは持続RMS・定常性)。
  //   最優先は「疲れないこと・主張しないこと」。BGMらしい旋律は置かない(SoundSkills §3)。
  //   惑星差は STAGES[].sky から導出する(色は既に惑星ごとの真実=音のために別表を作らない)。
  //   調整は個別惑星ではなく**下の範囲値**で行う=10惑星の関係(明るい/暗い・暖色/寒色)が崩れない。
  soundAmbOn: true,            // 環境音だけのON/OFF(音全体とは別。falseで環境音だけ止まる=可逆)
  soundAmbLevel: 0.05,         // ★環境音の基準音量。SE最軽(給餌peak0.080)よりさらに小さく置く
  soundAmbWindLevel: 1.0,      // ★風ノイズの比率
  soundAmbPadLevel: 0.35,      // ★微パッドの比率(風より控えめ=パッドが前に出ると主張になる)
  soundAmbNoiseSec: 3.0,       // 風ノイズのループ長秒(短いと周期が耳につく)
  soundAmbSeed: 424242,        // 決定論ノイズの種(同一設定=同一波形)
  soundAmbCutMinHz: 320,       // ★風のローパス下限(暗い空=こもった風)
  soundAmbCutMaxHz: 1400,      // ★上限(明るい空=乾いた風)
  soundAmbBreathSec: 14,       // ★風の息づかいの周期秒。短いと疲れる(常時鳴るため特に重要)
  soundAmbBreathDepth: 0.35,   // ★息づかいの深さ。深いと主張する
  soundAmbPadBaseHz: 110,      // ★パッドの基準周波数
  soundAmbPadOctaves: 2,       // ★空の明るさで振るオクターブ幅(暗い惑星は低く沈む)
  soundAmbPadDetune: 1.008,    // ★2声のずれ(うなり=ゆっくりした揺らぎ)
  soundAmbFadeSec: 1.2,        // ★惑星切替のフェード秒(下げてから差し替えて戻す=クリックもポルタメントも出さない)
  soundAmbRenderSec: 4.0,      // テスト器: 環境音のレンダリング尺(定常性を見るため数秒必要)
  soundAmbLowBandHz: 500,      // テスト器: 低域比率を測る一次ローパスの遮断周波数
  soundAmbSteadyWinMs: 100,    // テスト器: 定常性を測る窓の長さms。長すぎると速い揺れを窓内で平均して見逃す
  soundAmbSteadySkipSec: 1.0,  // テスト器: 定常性から外す立ち上がり秒(巣の反響が積み上がるまで。フェードイン中に隠れる)
  // ---- V6-P5 S3: 場所の環境音(本部/巣)。★全数値はRicが ?tune=1#sound で実機判定 ----
  //   飼育槽と同じ物差し(持続RMS・定常性・低域比率)で測り、同じ基準音量(soundAmbLevel)から出発する。
  //   ★場所の音程・こもりは既存の真実から導出する(本部=CFG.holoPal.void / 巣=NEST_VIS.palette.bg0)。
  //     ここにあるのは**質感**(構造)の調整値だけ=場所ごとの音程表は存在しない。
  //   ★持続層(ハム/パッド)と点の層(tick/心拍)で物差しが違う: 定常性は持続層、点の層はpeak。
  soundHqHumLevel: 0.36,       // ★本部: 低ハムの比率(強フィルタ矩形波=計器のうなり)。飼育槽の持続RMSと同程度に置く
  soundHqTickGrids: 8,         // ★本部: tickの周期 = CFG.holoGridSec × これ(起動シーケンスと同じ0.4秒グリッドに乗る)
  soundHqTickHz: 2400,         // ★本部: tickの高さ(計器の点。高く短いほど「冷たい」)
  soundHqTickSec: 0.012,       // ★本部: tickの長さ秒(可聴限界近くの点=主張させない)
  soundHqTickLevel: 0.45,      // ★本部: tickの比率。これを0にすると持続層だけになる(定常性の計測で使う)
  soundNestPadLevel: 0.9,      // ★巣: ディレイパッドの比率
  soundNestPadDetune: 1.0012,  // ★巣: 2声のずれ。★飼育槽(1.008)より小さいのは、巣はパッドが主役で
                               //   うなりを風が覆い隠さないため。1.008 では うなり周期1.85秒=「速い揺れ」になり
                               //   定常性3.48(閾値3.2超)で実際に落ちた。周期を10秒以上へ延ばして是正(§3-2の規律)。
  soundNestDelaySec: 0.42,     // ★巣: ディレイ時間秒(籠もりの長さ)
  soundNestFeedback: 0.55,     // ★巣: 反響の残り具合(1未満=発散しない)
  soundNestPulseSec: 4.4,      // ★巣: 心拍の周期秒。安息なので非常にゆっくり(速いと主張になる)
  soundNestPulseGap: 0.28,     // ★巣: 2打目までの間(ドッ・ドッ)
  soundNestPulseDur: 0.16,     // ★巣: 1打の長さ秒
  soundNestPulse2Amp: 0.62,    // ★巣: 2打目の強さ(1打目より弱い=心拍らしさ)
  soundNestPulseRatio: 0.5,    // ★巣: 心拍の高さ = パッド音程 × これ(1オクターブ下)
  soundNestPulseLevel: 0.5,    // ★巣: 心拍の比率。これを0にすると持続層だけになる(定常性の計測で使う)
  // ゲームイベント → 音id の対応表(V6-P5 S1配線・**唯一の変換点**)。
  //   ルール層は Game.emit(name) で「何が起きたか」だけを流し、ここで「何を鳴らすか」に変換する。
  //   ★対応表に無いイベントは鳴らない=音を足すにはまずSoundSkills §4の対象表へ追記し、ここに1行足す。
  //   ★憲章§2-2: 各イベントは既存の視覚表示と対(feed=+xpポップ / hatch=登場エフェクト / defeat=撃破通知)。
  //   ★S3: ルール層のイベント源は Game だけではない。四重スリット(Slit)も自前のイベント列を持つ
  //     (Slit.drainEvents)。**変換点を2つに割らない**ため、どちらの源のイベントもこの1つの表で引く。
  soundCues: { feed: "feed", hatch: "hatch", defeat: "defeat", SlitSuccess: "stone" },
  ...
  soundDefs: {
    ...
    // 賢者の石の生成(=四重スリットの全通過)。優先度表の最上位(stone:5)であり、**この作品で最も重い一撃**。
    //   ★全通過の正値は 1/4339(装置設計値・S-SLIT-R後の実測が整合。≈1/3900は旧MC値)。極めて稀な事象なので、ここに音予算を集中してよい(§2-7 核となる瞬間)。
    //   ★2層: ノイズバースト(衝撃)+ ベル(長い余韻)。稀さに見合う長い尾を持たせる=最も長い音でもある。
    //   ★序列: peak は撃破(0.259)を明確に超える。ただし**未実装のボス撃破(優先度4)が撃破と石の間に
    //     入る余地**を残してある(石を撃破のすぐ上に置くとボスの居場所が無くなる)。
    stone: {
      dur: 0.10, vol: 0.95, attack: 0.002, release: 0.34,
      layers: [
        { type: "noise", filterType: "lowpass", filterFreq: 2400, seed: 20260813 },   // 衝撃
  ...
  hoshiNight: 0.36,         // 腹側の夜藍の沈み(星の下地=暗さが先・基準の同系色文法)
  hoshiLine: 0.45,          // 等星を結ぶ航路線の濃さ(0=無効)
  kokePatches: 3,           // 苔斑の枚数
...
const NEST_VIS = {
  ...
  // B4改訂: core.x は**グラフ領域基準**(参照の右パネル列 x≈1078 を除いた 0〜1066)= 618/1066=0.580。
  //   実ページのステージも右パネルを除いた領域のため、これで参照と構図が一致する(B0時の0.423は
  //   パネル込み全幅基準の誤りだった=ステージにパネルが無い前提でしか合わない)。
  core: { x: 0.580, y: 0.410, r: 0.153 },   // 巣コア(編み枝の鉢)の中心と半径(実測: 鉢≈(830,420) 半幅≈135px)
