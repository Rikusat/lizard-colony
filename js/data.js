...
const CFG = {
  ...
  // ── 二重スリット実験的瞬間遺伝決定装置(roulette.md §9・左メニュー下部・クランク稼働で作動) ──
  ...
  // 各円は独立回転する(§9.1新)。回転で全スリット同時整列が必要=通過は各円ほぼ独立→成功≈∏(半角/180)。
  slitHalfDeg: [54, 50, 46, 42],       // 各円の切れ目の半角(度)。外=54(通過30%=70%弾き)。回転MCで確定:
                                       // 成功1.16%(≈1/86)・弾かれ ring1:70/ring2:22/ring3:6/ring4:0.7・ring2到達30%(惜しさ保持)
  slitSpinDeg: [6, -9, 11, -14],       // 各円の回転速度(度/秒・符号=向き)。円ごとに違う速さ/向き=切れ目が常にズレ、たまに揃う。
                                       // 目立たない低速(1周26〜60秒)。時間の純関数で決定論(固定dt累積)。ウォールクロック非依存
  ...
  slitCooldownSec: 2.5,    // クランク稼働時の作動クールダウン(§9.3-1)。小窓で点が一瞬走るだけなので頻繁でよい(実機調整)
  // 失敗球の張り付き(学習性無力感の防止=失敗を"どこまで潜ったかの痕跡"として残す・§C-slit.3-2)
  slitStickSec: 60,        // 張り付いた球が残る秒数(頻度up=速く溜まるので寿命は短め。実機調整)
  slitStickMax: 22,        // 同時に残せる最大数(窓拡大で余地増。超過は外側=浅い/古い順で消し、内側=惜しい記録を優先残置)
