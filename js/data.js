...
const CFG = {
  ...
  //   shape=リングの姿形。"ring"=円弧(原型・A) / "poly"=多角形枠(B) / "segment"=分節(C) / "organic"=有機曲線(D) / "double"=二重(E) / "sign"=標識・警戒縞(F)。
  //     いずれも切れ目は「角度」で飛ばす=物理の角度窓(slit.js: 固定半径跨ぎ時に角度のみ照合)と厳密一致=見た目の隙間=粒子が通れる角度。
  //     B: sides=辺数(頂点が物理半径radii[i]に接し辺はcos(π/sides)へ窪む)。C: segStepDeg=粒の間隔/segGapFrac,segGapMaxDeg=粒の隙間(物理の切れ目より必ず小さく保つ=切れ目が曖昧にならない)。
  //     D: wobAmp/wobLobes(+第2波wobAmp2/wobLobes2)=揺らぎ(半径が視覚上ズレない様、振幅は数%)。E: dblSep=内外の間隔。F: dashPx=警戒縞・centerShape="trefoil"=三葉。
  //     lwMul=線の太さ倍率。各リングは物理の独立回転(slitSpinDeg)で枠ごと回る=「回転しあう幾何」。骨格(半径/切れ目角/回転)は不変=意匠のみ。
  ...
  slitSkinByStage: {
    ...
    5: { shape: "segment", segStepDeg: 11, segGapFrac: 0.3, segGapMaxDeg: 4, lwMul: 1.55, rail: [178, 110, 76], glow: [255, 150, 60], center: [255, 172, 82], trace: [206, 124, 78], traceRing: [255, 160, 70], laser: [255, 192, 112], bloom: [255, 140, 60] }, // イグニス=C分節(黒殻の断片列・隙間から熔岩の橙が覗く)。熔岩橙×黒殻
    ...
    7: { shape: "double", dblSep: 0.045, rail: [140, 208, 200], glow: [200, 240, 236], center: [180, 232, 224], trace: [160, 214, 206], traceRing: [190, 236, 228], laser: [216, 248, 244], bloom: [176, 230, 220] }, // メアリス=E二重(同心の二重線が波紋のように)。真珠青緑
    ...
    9: { shape: "sign", dashPx: [8, 6], lwMul: 1.4, centerShape: "trefoil", rail: [150, 158, 166], glow: [130, 225, 190], center: [140, 232, 198], trace: [170, 178, 186], traceRing: [140, 228, 194], laser: [188, 244, 222], bloom: [140, 232, 198] }, // ヴォルタ=F標識(警戒縞の枠+放射線標識の三葉が回転)。鈍色×被曝冷光
    10: { shape: "segment", segStepDeg: 26, segGapFrac: 0.34, segGapMaxDeg: 4.5, lwMul: 2.2, segCap: "round", rail: [196, 150, 96], glow: [230, 180, 110], center: [240, 200, 130], trace: [202, 162, 110], traceRing: [235, 190, 120], laser: [245, 210, 150], bloom: [230, 176, 106] }, // オリジン=C分節(立石が環状に並ぶ・石の間が切れ目)。琥珀×土
  },
