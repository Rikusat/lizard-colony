...
if (typeof location !== "undefined" && /[?&]tune=1(?:&|$)/.test(location.search)) {
  ...
  { // #roster: 初回ロード時+hashchange両対応(同一URLのハッシュ変更はリロードされないため=調査で特定した起動漏れの根治)
    const buildRoster = () => {
      ...
      // P3-1裁定①(2026-08-11): 収集/整列/表生成は本部の共通関数 UI.buildRosterTable へ委譲=単一の真実・二重実装なし。
      //   devはSAVE同定ヘッダ(環境切り分け)だけを上乗せ(本番パネルには出さない)。
      let ident = "";
      try {
      ...
      host.innerHTML = ident + `<div style="margin-bottom:6px"><b style="font-size:15px">個体一覧(dev・読み取り専用)</b> <span style="color:#a89a83">URLの#rosterを外すと閉じる</span></div><div id="dev-roster-host"></div>`;
      UI.buildRosterTable(host.querySelector("#dev-roster-host"), host._rst || (host._rst = { sort: "default", dir: 1, fSp: "", fMo: "", fTr: "", fPl: "" }));
    };
    const rosterGate = () => {
    ...
    //   再生は本ビューア専用のrAF=本編描画ループに条件分岐を足さない。
    {
      ...
      //       (4秒待てば 4/4 表示。4経路=初回/ハッシュ追記/リロード/タブ復帰 で実測済)。
