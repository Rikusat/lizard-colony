...
const Game = {
  ...
  checkDaily() {
    ...
    setTimeout(() => UI.toast(`📅 デイリーボーナス! ${s.daily.streak}日連続 → +${fmt(coins)}G +💎${gems}`), 800);
  },
  ...
  forgeFacility(id) {
    ...
    UI.toast(`⛏️ ${f.name}が化けた! Lv上限+1(現在 上限${this.facMax(f)})`);
    return true;
  },
  ...
  investRocket(n) {
    ...
    if (r.invested >= this.rocketStageNeed()) {
      ...
      UI.toast(`⚙️ イリジウム${amount}を投入(${r.invested}/${this.rocketStageNeed()})`);
    }
    return true;
  },
  ...
  convertBio() {
    ...
    UI.toast(`🏰 🧬${fmt(need)} → 🔬研究力+${CFG.convertBatch} に変換した`);
    return true;
  },
  ...
  mutateLizard(lz) {
    ...
    this.flashT = 0.4;
    UI.toast(`🧬 突然変異!! ${this.lizardName(lz)} へ姿を変えた!`);
    return true;
  },
  ...
  registerDex(speciesId, morphId, silent) {
    ...
    if (!silent) UI.toast(`📖 図鑑に新登録! ${sp.name}(${mo.name}) +💎${gems}`);
  },
...
;
  ...
  buyResearch(id) {
    ...
    s.research[id] = true;
    UI.toast(`🔬 研究完了「${r.name}」— ${r.desc}(全Stage恒久)`);
    return true;
  },
    ...
    if (!e.arrived) {
      ...
      if (e.x <= SNAKE_HOME.x) {
        ...
        if (trap > 0) {
          ...
          e.hp -= dmg;
          this.popup(e.x, e.y - 50, `⚔️罠 -${fmt(dmg)}`, "#ffb060");
        }
  ...
  endRaid(win, reason) {
    ...
    if (win) {
      ...
      if (r.tier) {
        ...
        this.addRes("bio", bio);
        msg = `🎉 BOSS撃破! ${r.elite ? "👑Elite " : ""}${r.type.name} T${r.tier} / +${fmt(coins)}G +💎${gems} +🔬${sci} +🧬${bio}`;
        ...
        msg = `🎉 蛇を撃退! +${fmt(coins)}G +💎${gems} (ボス討伐!)`;
        this.addRankXp(100);
      } else {
        msg = `🎉 蛇を撃退! +${fmt(coins)}G`;
        if (Math.random() < 0.2) { s.gems += 1; msg += " +💎1"; }
        this.addRankXp(30);
      }
  ...
  popupBurst(x, y) {
    for (let i = 0; i < 12; i++) {
      this.popup(x + rnd(-70, 70), y + rnd(-40, 30), ["💰", "+G", "✨"][i % 3], "#ffd24c");
    }
  },
  ...
  rollMerchant() {
    ...
    offers.push({ kind: "bio", label: "🧬 生態データ ×80", price: 6 });
      ...
      if (data.version >= 4) {
        ...
        setTimeout(() => {
          ...
          if (r && (r.amber > 0 || r.iridium > 0)) {
            UI.toast(`💱 旧探索の払い戻し: 🟠琥珀+${r.amber} ⚙️イリジウム+${r.iridium} ☄️隕石+${r.meteorite} 🔩オリハルコン+${r.orichalcum}`);
          }
        }, 900);
        ...
        setTimeout(() => {
          ...
          if (r && (r.refundG > 0 || r.bio > 0)) {
            UI.toast(`💱 素材システム廃止にともない払い戻し: +${fmt(r.refundG)}G / 🧬+${fmt(r.bio)} / 🔬+${fmt(r.science)}`);
          }
        }, 900);
      } else {
        // V2以前 → v2 → V4 の二段移行
